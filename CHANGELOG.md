...
- 🟢 **符号索引影子构建（#38）**：索引重建写入影子副本、完成后原子切换——重建期间旧索引/磁盘缓存持续可查，修复"重建窗口期跨文件跳转全黑"；当前打开文件的符号解析后即时并入索引（无需等仓库扫描完成）；构建期间到达的文件变更进入待处理队列、切换前收敛式重放
- 🟢 **索引状态栏三态（#38）**：状态栏常驻 `PL/SQL 跳转未启用`（点击打开设置）/ `索引构建中 x/y`（实时进度）/ `索引就绪 · N 符号`（点击查看日志）；构建中未命中跳转时提示"索引构建中，跨文件跳转暂不可用"；手动"重建索引"带真实进度、可取消（取消保留旧索引）、完成后落盘
- 🟢 **索引大幅提速（#39）**：新增轻量符号扫描器（不建大纲树，清洗层与 CREATE/子程序匹配复用解析器同一正则事实源；corpus 全量一致性对照锁定）——首次全量建索引约 7 倍提速（143k 行语料 803→109ms）；mtime+size 增量构建——重启/改配置只重扫变化文件，5000 文件仓库无变化重建 <50ms；文件读取分块并行；解析器清洗层快路径使大纲解析本身约 2.8 倍提速（万行文件 20–65ms → 16–38ms）
- 🟢 **Ctrl+T 全仓符号搜索（#39）**：基于索引按名搜索全部代码仓库的包/函数/过程/触发器/**游标**，支持 `pkg.func` 双重过滤；游标为"仅搜索条目"（包内带所属包、可定位声明行，但不参与 Ctrl+Click 跳转解析，跳转语义零漂移）
- 🔴 **修复（#38）**：缓存加载后 watcher 更新产生重复符号条目（缓存格式升 v4：符号 + 文件映射 + 每文件 mtime 新鲜度标记，旧格式自动失效重建一次）；带解析错误的文件不再被整体丢弃符号；配置变更后 watcher 仍监视旧路径（重建后同步刷新）；带引号标识符等暂不支持语法导致的整文件符号缺失（best-effort 提取）
- 🧪 新增 `symbol_scanner_test`（15：定向形态 + corpus 一致性对照）、回归 symbolIndex 测试 17–21（双路径冲突/增量构建/旧格式拒绝/搜索/游标仅搜索）、E2E `indexJump`（11：双仓库路径真实跳转全链路——优先级消解/包名限定/watcher 增量/Ctrl+T）、压力测试脚本 `tests/stress/index_stress.js`（11 阶段断言，2000/5000 文件规模全绿）；单元基线 489/489(28) → **504/504(29)**，回归 14/14，e2e +indexJump 11/11

## v1.14.0 (2026-09-22)

- 🟢 **大纲搜索过滤（#36）**：大纲树上方新增常驻搜索框（Webview 视图），输入实时过滤（200ms 防抖）——按节点名/声明项名子串匹配（大小写不敏感），命中项及其祖先链保留、其余隐藏；过滤期间命中分支自动展开（TreeItem.id 加 `::f` 后缀获得独立展开状态）、标题栏显示命中数、光标跟随暂停；回车跳转首个命中，Esc/✕ 清除并完整恢复原展开状态（搜索框可通过视图菜单隐藏）
- 🟢 **逐级展开（#36）**：控制结构（含 ELSIF/ELSE 合并 IF）与区域文件夹默认折叠——展开父级只露出一级，子层级逐级手动展开（此前控制结构默认全展开，展开 Body 即级联展开整棵子树）；顶层对象（包等）沿 `view.expandByDefault`（默认开）展开一层；「展开所有」命令补齐为文件夹/代码单元也强制展开
- 🟢 **嵌套缩进改原生树缩进（#36，推翻 #33 的 NBSP 标签伪缩进）**：图标随层级移动、图标-文字间距固定、参考线落在图标上；缩进宽度默认 8→16px（`workbench.tree.indent`，与 `renderIndentGuides=always` 一并经 package.json `configurationDefaults` 提供，用户显式配置优先——同时取代 v1.13.0 的激活时写入用户设置，不再污染 settings）；`TreeItemData.displayIndent` 字段删除
- 🟢 **悬浮聚合游标 SQL（#36）**：悬浮大纲单元节点（过程/函数/包/触发器等）或编辑器中的单元名，聚合显示该单元直接作用域内声明的全部游标 SQL（逐条 ```sql 代码块、按声明行序、高度随数量自适应；嵌套子程序作用域不并入父级）；无游标单元显示简洁头信息，控制结构占位名不触发
- 🟢 **右键复制名称（#36）**：大纲项右键「复制名称」把干净标识符（node.name / 声明项名，不含类型后缀与行号）写入剪贴板；控制结构占位名（IF/LOOP 等）与文件夹提示不可复制；命令已从命令面板隐藏
- 🧪 新增 `outline_filter_test`（22）、`control_expand_test`（17，取代 control_indent_test）、`copy_name_test`（8）、`node_sql_hover_test`（15）；activation/refresh_focus 回归 mock 补 `registerWebviewViewProvider`；单元基线 438/438(25) → **489/489(28)**，回归 14/14 不变
