...
| 2 | `npm test`（= compile + tests/unit/run_all.js） | **385/385（22 套件）** | 解析/渲染/导航/取消/设置/引号标识符/get_ddl/折叠 契约 |
| 3 | `npm run test:regression` | **13/13 套件** | 真实世界包/超大文件/边界/光标同步/定义跳转/符号索引/内存/Refresh 焦点回退/激活自动解析 |
| 4 | `npm run test:e2e` | **anonDefinition 通过 + smoke 7/7 + foldRouting 3/3** | 真实 VS Code 宿主（@vscode/test-electron）；foldRouting 锁定配置扩展名文档的提供者路由（Issue #26） |
