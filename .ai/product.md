...
- **符号索引可用性**（Issue #38）：索引影子构建 + 原子切换——重建期间旧索引/磁盘缓存
  持续可查，不再出现"重建窗口期跳转全黑"；当前打开文件的符号在解析后即时并入索引
  （无需等仓库扫描完成）；状态栏常驻三态项「PL/SQL 跳转未启用 / 索引构建中 x/y /
  索引就绪 · N 符号」（点击：未配置→打开设置页，已配置→查看日志）；构建中未命中跳转
  时状态栏提示"索引构建中，跨文件跳转暂不可用"；手动重建带真实进度、可取消、完成落盘。
