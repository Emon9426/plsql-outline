...
- **控制结构分层**：IF/ELSIF/ELSE、LOOP/WHILE/FOR、CASE/WHEN 按代码层级缩进展示（循环内的 IF 相对循环缩进），支持 3 级以上嵌套子程序
- **结构块显示**：BEGIN、EXCEPTION、END 等结构块可开关（`view.showStructureBlocks`）
- **光标联动**：在编辑器里移动光标，大纲自动选中光标所在的节点；反之点击节点跳转代码
...
- **一键展开/折叠全部节点**、手动刷新、调试模式（输出详细解析日志到输出面板）
