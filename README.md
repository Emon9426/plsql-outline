...
| `Ctrl+T` 输入符号名 | 全仓搜索代码仓库里的包/函数/过程/触发器/**游标**，支持 `pkg.func` 双重过滤；游标仅作搜索定位，不参与 Ctrl+Click 跳转解析 |

跨文件跳转依赖"代码仓库路径 + 符号索引"，在[配置项](#-配置项)的 `codeRepository` 部分设置。

**符号索引与全仓搜索（v1.15.0 起）**

- **影子构建**：索引重建期间旧索引持续可查，跨文件跳转不再出现"重建中不可用"；当前打开文件的符号解析后即时并入索引
- **状态栏三态**：`PL/SQL 跳转未启用`（点击打开设置）/ `索引构建中 x/y`（实时进度）/ `索引就绪 · N 符号`（点击查看日志）；构建中跳转未命中时明确提示原因
- **增量构建**：文件未变（mtime+size）即跳过重扫——重启后索引秒级就绪、修改配置增量迁移；5000 文件仓库首次建索引约 1 秒
- **手动重建**：带真实进度、可取消（取消保留旧索引）、强制全量重扫、完成后落盘缓存
...
- 万行文件毫秒级解析（10,000 行实测 16–38ms；清洗层快路径使整体解析较上版约 2.8 倍提速）
...
- 35 文件 / 14 万行结构化测试语料保障解析正确性；符号索引另有 5000 文件 / 16 万行
  合成仓库压力测试（首建 ~1s / 增量重建 <50ms / 双仓优先级冲突消解）保障
