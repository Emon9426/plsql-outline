...
- **游标 SQL 悬浮**：鼠标悬浮大纲里的游标项（或编辑器内的游标名），直接显示该游标的完整 SQL 原文
- **控制结构分层**：IF/ELSIF/ELSE、LOOP/WHILE/FOR、CASE/WHEN 按代码层级缩进展示——嵌套结构每层明显缩进（标签缩进 + 缩进参考线），ELSIF/ELSE 分支内的控制结构同样可见；支持 3 级以上嵌套子程序
