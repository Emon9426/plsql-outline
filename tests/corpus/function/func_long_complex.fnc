-- =============================================================================
-- 用例: Function / 长代码+复杂结构 (function/func_long_complex.fnc)
-- 本文件由 ZCodeTest/generate-long.js 确定性生成(10,000+ 行), 请勿手工编辑;
-- 再生: node ZCodeTest/generate-long.js
-- 覆盖: 多行签名
-- 覆盖: 声明区全家桶+嵌套子程序3层(过程>函数>过程)
-- 覆盖: 普通段落(IF/FOR/WHILE/内联块)与复杂段落轮转:
-- 覆盖: 3层循环/注释风暴(单行+块注释+注释掉的代码)/内联匿名块/Q-quote+CASE ELSE/游标FOR
-- 覆盖: EXCEPTION 多 WHEN
-- =============================================================================
CREATE OR REPLACE FUNCTION fn_long_complex_calc(
    p_start   IN  NUMBER,
    p_step    IN  NUMBER,
...
IS
    ...
    PROCEDURE sub_calc_fn(
        ...
        FUNCTION sub_format_fn(p_v IN NUMBER) RETURN VARCHAR2 IS
            v_txt  VARCHAR2(100);

            -- ----- 嵌套子程序 第 3 层: 子过程(位于子函数内) -----
            PROCEDURE sub_check_fn(p_v2 IN NUMBER) IS
            BEGIN
                IF p_v2 < 0 THEN
                    RAISE e_invalid;
                END IF;
    ...
    BEGIN
        ...
        v_pool_0001 := v_pool_0001 + LENGTH(sub_format_fn(v_step));
        p_out := v_step;
    EXCEPTION
        WHEN e_invalid THEN
            p_out := -1;
